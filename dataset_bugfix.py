...
Image.MAX_IMAGE_PIXELS = None  # 제한을 없앰
@njit
...
class SceneTextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for word_info in self.anno['images'][image_fname]['words'].values():
            ...
            if num_pts > 4:
                continue
            if(image_fpath.split('.')[1][:2] == 'th'):
                vertices.append((np.array(word_info['points'])*4).flatten()) ###여기만 내가 수정
            else :
        ...
        if(image_fpath.split('.')[1][:2] == 'th'):
            modified_image_fpath = f"{image_fpath}_DRCT-L_X4.png" ###여기만 내가 수정
        else :
            modified_image_fpath=image_fpath
        image = Image.open(modified_image_fpath)
