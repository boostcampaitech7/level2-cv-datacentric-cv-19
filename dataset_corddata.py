...
class SceneTextDataset(Dataset):
    def __init__(self, root_dir,
        ...
        json_file_path = "/data/ephemeral/home/newdataset/CORD/newtrainjson/combined.json"
...
# JSON 파일 읽기
        with open(json_file_path, 'r', encoding='utf-8') as f:
            ...
            for im in anno['images']:
                # 이미지 파일 경로
                image_fpath = osp.join("/data/ephemeral/home/newdataset/CORD/train/image", im)
                
                # 이미지 파일 확인
                if not osp.isfile(image_fpath):
                    print(f"Warning: Image file not found at {image_fpath}. Skipping this image entry.")
                    continue  # 이미지가 없으면 해당 이미지는 건너뜁니다.

                # 이미지가 존재하는 경우에만 total_anno에 추가
                total_anno['images'][im] = {
                    "paragraphs": anno['images'][im]["paragraphs"],
                    "words": {}
                }
    ...
    def __getitem__(self, idx):
        ...
        image_fpath = osp.join("/data/ephemeral/home/newdataset/CORD/train/image", image_fname)
